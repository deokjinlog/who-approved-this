...
def run(
    ...
    if track == "t1s":
        t1s_summary.report(TRACKS[track](model=model, max_docs=docs))
        return
    ...
    if track == "t1g":
        t1g_agent_eval.report(TRACKS[track](model=model, max_docs=docs))
        return

    if track == "t1f":
        t1f_agent.report(TRACKS[track](model=model, max_docs=docs))
        return
